...
const SuggestionList = ({ suggestion, selected, selectSuggestion, getValue }) => {
  ...
  return (
    ...
    >
      ...
      <p className='font-semibold text-lg capitalize text-green-500 px-3' onClick={handleClick}>
...
const MenuSearchBar = ({ handleSearch }) => {
  const { changeSearchText, search, suggestions, selectSuggestion, selected } = useSearch(process.env.REACT_APP_BASE_URL+'/products/search');
  ...
  const handleKeyDown = (e) => {
    if (e.key.toLowerCase() === 'arrowdown') {
      ...
      if (highlightedIndex >= 0) {
        ...
        handleSearch(selectedSuggestion.name,selectedSuggestion._id);
      }
    }
  };
          ...
          {suggestions?.length !== 0 ? (
            suggestions.map((suggestion, ind) => (
              <SuggestionList
                ...
                getValue={() => handleSearch(suggestion.name,suggestion._id)}
